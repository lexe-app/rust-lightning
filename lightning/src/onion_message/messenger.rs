	...
	CMH::Target: CustomOnionMessageHandler,
{
	...
	pub fn new(
		...
		Self::new_inner(
			...
			offers_handler, async_payments_handler, dns_resolver, custom_handler,
				...
				if self.queue_messages_for_offline_peers {
					match message_recipients.entry(next_node_id) {
						...
						hash_map::Entry::Vacant(e) => {
							log_trace!(logger, "Forwarding onion message to disconnected peer {}: awaiting connection", next_node_id);
							let addrs = Vec::new();
